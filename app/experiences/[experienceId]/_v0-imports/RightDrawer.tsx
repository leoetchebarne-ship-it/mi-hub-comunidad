      ...
      >
        ...
        <div className="p-4 space-y-4 overflow-y-auto h-[calc(100%-60px)]">
          <div className="text-xs text-white/40 space-y-1">
            <p>Semana {selectedWeek} • {currentStage}</p>
          </div>
          ...
          {isAdding ? (
            <div className="space-y-3 p-3 border border-electric-blue/30 rounded-lg bg-electric-blue/5">
              <input
                ...
                className="w-full bg-transparent border-b border-white/20 text-sm text-white focus:outline-none"
              />
              <div className="flex gap-2">
                <button onClick={handleSaveNote} className="flex-1 py-2 text-xs bg-electric-blue/20 text-electric-blue border border-electric-blue/30 rounded">GUARDAR</button>
                <button onClick={() => setIsAdding(false)} className="px-3 text-xs text-white/40">X</button>
              </div>
            </div>
          ) : (
            <button
              onClick={() => setIsAdding(true)}
              className="w-full py-4 border border-dashed border-white/20 rounded-lg text-xs text-white/40 hover:text-electric-blue/70 transition-all"
            >
              + NUEVA NOTA
            </button>
          )}
